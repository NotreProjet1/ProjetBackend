...
const createFormation = async (req, res) => {
    try {
        ...
        authenticateToken(req, res, async () => {
            const { titre, description, contenu, domaine, plant, certeficat } = req.body;

            const result = await FormationModel.createFormation(titre, description, contenu, domaine, plant, certeficat);
...
const updateFormation = async (id_fp, titre, description, contenu, domaine, plant, certeficat) => {
    const updateQuery = `
        UPDATE formation_p
        SET titre = ?, description = ?, contenu = ?, domaine = ?, plant = ?, certeficat = ?
        WHERE id_fp = ?
    `;

    await db.query(updateQuery, [titre, description, contenu, domaine, plant, certeficat, id_fp]);
